...
int allocateHostMemoryForComputation(float **lambdaDiff2Pointer, float **qImageArrayPointer, float **uImageArrayPointer,
									 float **qPhiPointer, float **uPhiPointer, float **pPhiPointer,
									 int nInFrequencies, int nRA, int nPhi,  int nInElements, int nOutElements){
	/* Allocate memory on the host */

	*lambdaDiff2Pointer = (float *)calloc(nInFrequencies, sizeof(**lambdaDiff2Pointer));
	*qImageArrayPointer = (float *)calloc(nInElements, sizeof(**qImageArrayPointer));
	*uImageArrayPointer = (float *)calloc(nInElements, sizeof(**uImageArrayPointer));
	*qPhiPointer = (float *)calloc(nOutElements, sizeof(**qPhiPointer));
	*uPhiPointer = (float *)calloc(nOutElements, sizeof(**uPhiPointer));
	*pPhiPointer = (float *)calloc(nOutElements, sizeof(**pPhiPointer));
...
int allocateDeviceMemoryForComputation(int cudaDeviceId, float **lambdaDiff2, float **phiAxis,
	...
	cudaMalloc(&phiAxis, sizeof(**phiAxis)*nPhi);
...
int copyLambdaDifferenceToDevice(int deviceId, float *lambdaDiff2, float *device_lambdaDiff2, size_t length){
	...
	cudaMemcpy(device_lambdaDiff2, lambdaDiff2, sizeof(*lambdaDiff2)*length, cudaMemcpyHostToDevice);
	checkCudaError();
...
int copyStepToHost(int deviceId, float *qPhi, float *d_qPhi, float *uPhi, float *d_uPhi, float *pPhi, float *d_pPhi, size_t length){
	int currentCudaDevice;
	// reads the current selected device and switch back after
	cudaGetDevice(&currentCudaDevice);
	// set the desired device
    ...
    cudaMemcpy(qPhi, d_qPhi, length*sizeof(*qPhi), cudaMemcpyDeviceToHost);
    cudaMemcpy(uPhi, d_uPhi, length*sizeof(*qPhi), cudaMemcpyDeviceToHost);
    cudaMemcpy(pPhi, d_pPhi, length*sizeof(*qPhi), cudaMemcpyDeviceToHost);
	checkCudaError();
...
int doRMSynthesis(struct optionsList *inOptions,
    ...
    // Dimension sizes
    int nFrequencies, nRa, nDec, nPhi;
    ...
    switch(inOptions->fileFormat) {
       ...
       case HDF5:
          ...
          descriptors->qDataspace = H5Dget_space(descriptors->qDataset);
          descriptors->uDataset   = H5Dopen2(descriptors->uFileh5, PRIMARYDATA, H5P_DEFAULT);
          descriptors->uDataspace = H5Dget_space(descriptors->uDataset);
          ...
          if( descriptors->qDataset<0 || descriptors->uDataset<0 || descriptors->qDataspace<0 || descriptors->uDataspace<0 || descriptors->qMemspace<0 || descriptors->uMemspace<0 )
          ...
          descriptors->qOutDataspace = H5Dget_space(descriptors->qOutDataset);
          descriptors->uOutDataset   = H5Dopen2(descriptors->uDirtyH5, PRIMARYDATA, H5P_DEFAULT);
          descriptors->uOutDataspace = H5Dget_space(descriptors->uOutDataset);
          descriptors->pOutDataset   = H5Dopen2(descriptors->pDirtyH5, PRIMARYDATA, H5P_DEFAULT);
          descriptors->pOutDataspace = H5Dget_space(descriptors->uOutDataset);
    ...
    /* Compute \lambda^2 - \lambda^2_0 once. Common for all threads */
	computeLambdaSquareDifference(lambdaDiff2, data_arrays->lambda2, params->lambda20, nFrequencies);
    ...
    for(j=1; j<=params->qAxisLen2; j++) {
       ...
       switch(inOptions->fileFormat) {
          ...
          case HDF5:
             ...
             uerror = H5Sselect_hyperslab(descriptors->uDataspace, H5S_SELECT_SET, offsetIn,
                                    NULL, countIn, NULL);
             h5ErrorQ = H5Dread(descriptors->qDataset, H5T_NATIVE_FLOAT, descriptors->qMemspace,
                                   descriptors->qDataspace, H5P_DEFAULT, qImageArray);
             h5ErrorU = H5Dread(descriptors->uDataset, H5T_NATIVE_FLOAT, descriptors->uMemspace,
       ...
       copyStepToHost(selectedDeviceInfo.deviceID, qPhi, d_qPhi, uPhi, d_uPhi, pPhi, d_pPhi, nOutElements);
